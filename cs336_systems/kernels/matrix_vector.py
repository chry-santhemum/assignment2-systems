...
def weighted_sum_bwd(
    x_ptr, w_ptr, y_grad_ptr,
    x_grad_ptr, temp_w_grad_ptr,
    x_stride_row, x_stride_dim,
    w_stride_dim,
    y_grad_stride_row,
    x_grad_stride_row, x_grad_stride_dim,
    temp_w_grad_stride_dim, temp_w_grad_stride_nrowtiles,
    ROWS, D, 
    ROWS_TILE_SIZE: tl.constexpr, D_TILE_SIZE: tl.constexpr,
):
    block_idx = tl.program_id(0)
    n_row_tiles = tl.cdiv(ROWS, ROWS_TILE_SIZE)

    y_grad_block_ptr = tl.make_block_ptr(
        y_grad_ptr,
        shape=(ROWS,),
        strides=(y_grad_stride_row,),
        offsets=(block_idx * ROWS_TILE_SIZE,),
        block_shape=(ROWS_TILE_SIZE,),
        order=(0,),
    )
    ...
    x_block_ptr = tl.make_block_ptr(
        ...
        shape=(ROWS, D),
        strides=(x_stride_row, x_stride_dim),
        offsets=(block_idx * ROWS_TILE_SIZE, 0),
        block_shape=(ROWS_TILE_SIZE, D_TILE_SIZE),
        order=(0, 1),
    )
    temp_w_grad_block_ptr = tl.make_block_ptr(
        temp_w_grad_ptr,
        shape=(D, n_row_tiles),
        strides=(temp_w_grad_stride_dim, temp_w_grad_stride_nrowtiles),
        offsets=(0, block_idx),
    ...
    x_grad_block_ptr = tl.make_block_ptr(
        x_grad_ptr,
        shape=(ROWS, D),
        strides=(x_grad_stride_row, x_grad_stride_dim),
        offsets=(block_idx * ROWS_TILE_SIZE, 0),
        block_shape=(ROWS_TILE_SIZE, D_TILE_SIZE),
        order=(0, 1),
    )
    ...
    for i in range(tl.cdiv(D, D_TILE_SIZE)):
        ...
        # write results
        tl.store(x_grad_block_ptr, x_grad_block, boundary_check=(0, 1))
        tl.store(temp_w_grad_block_ptr, w_grad_block, boundary_check=(1,))
...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def backward(ctx, y_grad):
        ...
        weighted_sum_bwd[(tl.cdiv(n_rows, ctx.ROWS_TILE_SIZE),)](
            ...
            grad_weight_temp.stride(0), grad_weight_temp.stride(1),
            ROWS=n_rows, D=D,
            ROWS_TILE_SIZE=ctx.ROWS_TILE_SIZE, D_TILE_SIZE=ctx.D_TILE_SIZE,
        )
        ...
        grad_w = grad_weight_temp.sum(axis=1)
